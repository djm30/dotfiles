...
reg_tab_bg = as_rgb(0x2E323B)
reg_tb_fg = as_rgb(color_as_int(opts.color8))

active_tab_bg = as_rgb(color_as_int(opts.color13))
active_tab_fg = as_rgb(color_as_int(opts.color0))

icon_fg = as_rgb(color_as_int(opts.color16))
icon_bg = as_rgb(color_as_int(opts.color13))
...
def get_battery_cells() -> list:
    try:
        # Run pmset command to get power information
        result = subprocess.run(["pmset", "-g", "batt"], capture_output=True, text=True)
        output = result.stdout

        # Return empty if no battery found
        if "Battery" not in output:
            return []

        # Parse the output
        status_line = output.split("\n")[1]
        percent = int(status_line.split("\t")[1].split(";")[0].strip("%"))

        # More precise status detection
        is_charging = (
            "charging" in status_line.lower()
            and "discharging" not in status_line.lower()
        )
        is_discharging = "discharging" in status_line.lower()
        power_source = output.split("\n")[0]
        is_on_ac = "AC Power" in power_source

        # Determine icon and color based on more accurate status
        if is_discharging or not is_on_ac:
        ...
        else:  # Connected but not charging (full battery)
            icon_color = PLUGGED_COLORS[
                min(PLUGGED_COLORS.keys(), key=lambda x: abs(x - percent))
            ]
            icon = PLUGGED_ICONS[
